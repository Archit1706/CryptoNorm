...
-   **Order-book gap detection + resync**, implemented correctly _per exchange_:
...
![cryptonorm system architecture](docs/architecture.png)
...
-   **Full L2 book maintenance** (not just top-N) so sequence-gap detection +
    ...
    is genuinely _per-exchange_ (counter / `U`-`u` / checksum), which only became
...
-   **Kraken WS v2** (`BTC/USD`, not v1's `XBT/USD`). Its depth book is
    fixed-size and pushes levels out of the window _without_ sending removals, so
